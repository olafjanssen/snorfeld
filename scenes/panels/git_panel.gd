...
func _update_file_list():
	...
	var root: TreeItem = file_list.create_item()
...
func _add_file_to_list(parent_item: TreeItem, file_path: String, change_type: String, is_staged: bool):
	var item: TreeItem = file_list.create_item(parent_item)
	...
	var button_icon: Texture2D = checkbox_icons["checked"] if is_staged else checkbox_icons["unchecked"]
...
func _on_file_selected():
	...
	var item: TreeItem = file_list.get_selected()
	if item:
...
func _on_stage_button_clicked(item: TreeItem) -> void:
	...
	if metadata:
		...
		# Toggle staged state
		var is_staged: bool = not file_staged_state[file_path]
		file_staged_state[file_path] = is_staged
...
func _on_commit_pressed():
	var message: String = commit_message.text.strip_edges()
	if message == "":
		_show_error("Please enter a commit message")
		return

	var success: bool = $GitService.commit(message)
	if success:
		commit_message.text = ""
		_update_file_list()
